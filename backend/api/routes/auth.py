...
async def auth_register(body: dict, response: Response):
    ...
    if not username or len(username) < 2 or len(username) > 30:
        return JSONResponse({"error": "Username must be 2-30 characters long"}, status_code=400)
    if len(password) < 4:
    ...
    if phone:
        try:
            ...
            return JSONResponse({"error": "Invalid phone number format"}, status_code=400)
    ...
    try:
        ...
        if normalized_phone and await _phone_exists(db, normalized_phone):
            await db.rollback()
            return JSONResponse({"error": "Username, phone number, or email already exists"}, status_code=409)
...
async def auth_reset_send_code(body: dict):
    ...
    if not await cursor.fetchone():
        return JSONResponse({"error": "This email is not registered"}, status_code=404)
