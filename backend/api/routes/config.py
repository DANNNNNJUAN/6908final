...
async def _get_or_bootstrap_active_config(mac: str) -> Optional[dict]:
    config = await get_active_config(mac)
    if config:
        return config

    logger.info("[CONFIG BOOTSTRAP] No active config for %s, creating defaults", mac)
    await save_config(mac, {"mac": mac})
    return await get_active_config(mac)
...
async def get_config(
    ...
    config = await _get_or_bootstrap_active_config(mac)
    if not config:
