...
async def unbind_user_device(mac: str, force: bool = False, user_id: int = Depends(require_user)):
    ...
    if result == "owner_has_members":
        return JSONResponse({"error": "The owner still has shared members. Add ?force=true to unbind and remove them all."}, status_code=409)
    return {"ok": True}
...
async def approve_device_request(request_id: int, user_id: int = Depends(require_user)):
    ...
    if not membership:
        return JSONResponse({"error": "Request not found or cannot be approved"}, status_code=404)
...
async def reject_device_request(request_id: int, user_id: int = Depends(require_user)):
    ...
    if not ok:
        return JSONResponse({"error": "Request not found or cannot be rejected"}, status_code=404)
    return {"ok": True}
...
async def remove_device_member(
    ...
    if not ok:
        return JSONResponse({"error": "Member not found or cannot be removed"}, status_code=404)
    return {"ok": True}
...
async def save_user_llm_config_route(body: dict, user_id: int = Depends(require_user)):
    ...
    if llm_access_mode not in allowed_modes:
        return JSONResponse({"error": f"Invalid llm_access_mode: {llm_access_mode}"}, status_code=400)
    ...
    for url_val, url_name in [(base_url, "base_url"), (image_base_url, "image_base_url")]:
        if url_val and not (url_val.startswith("http://") or url_val.startswith("https://")):
            return JSONResponse({"error": f"{url_name} must start with http:// or https://"}, status_code=400)
...
async def redeem_invite_code(body: dict, user_id: int = Depends(require_user)):
    ...
    if not invite_code:
        return JSONResponse({"error": "Invitation code is required"}, status_code=400)
    ...
    try:
        ...
        if not row:
            await db.rollback()
            return JSONResponse({"error": "Invalid invitation code"}, status_code=400)
        if row[2]:  # is_used
            await db.rollback()
            return JSONResponse({"error": "Invitation code has already been used"}, status_code=409)
        ...
        return {
            "ok": True,
            "message": "Invitation code redeemed successfully. You received 50 free LLM calls.",
            "free_quota_remaining": quota.get("free_quota_remaining", 0) if quota else 0,
        }
