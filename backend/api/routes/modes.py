...
_PLACEHOLDER_SAMPLES = {
    "title": "Sample Title",
    "subtitle": "Subtitle",
    "text": "This is sample body text for layout preview.",
    "body": "Body content preview text goes here.",
    "quote": "Life is not about waiting for the storm to pass. It is about learning to dance in the rain.",
    "author": "Anonymous",
    "source": "Sample Source",
    "summary": "Summary preview text.",
    "question": "What question would you like to reflect on today?",
    "answer": "This is a sample answer used to demonstrate the layout.",
    "interpretation": "Interpretation preview text.",
    "challenge": "Daily challenge: try something new.",
    "word": "InkSight",
    "pinyin": "mò yú",
    "phonetic": "/ˈsæm.pəl/",
    "meaning": "Sample meaning",
    "definition": "A sample text used for layout preview.",
    "example": "Sample example sentence",
    "tip": "Tip: adjust the parameters to change the layout.",
    ...
    "postscript": "P.S. This is a sample postscript.",
    "hint": "Hint: the answer is closer than you think.",
    "note": "Note: this is sample annotation text.",
    "antidote": "Try looking at the problem from the other person's perspective.",
    "lines": ["The sun beyond the mountains glows", "The Yellow River seawards flows", "You can enjoy a grander sight", "By climbing to a greater height"],
    "message": "There are still this many days until the important dates...",
    "events": [{"name": "New Year", "date": "2027-01-01", "days": 266}, {"name": "Birthday", "date": "2026-08-15", "days": 127}],
    "days": 42,
}
...
async def custom_mode_preview(
    ...
    try:
        ...
        # 是否使用用户自带模型（profile / 设备配置），为 True 时不扣费不拦截额度。
        # custom_openai 只要配置了 base_url，也视为用户自带模型；本地网关可无真实 key。
        ...
        using_user_key = effective_api_key is not None or (
            user_llm_access_mode == "custom_openai" and user_llm_base_url is not None
        )
        ...
        if (
            ...
            if user_role != "root":
                ...
                if remaining <= 0:
                    return JSONResponse(
                        {"error": "Your free quota has been exhausted. Redeem an invitation code to continue."},
                        status_code=402,
                    )
...
async def generate_mode(
    ...
    try:
        ...
        if effective_provider == "openai_compat" and not (effective_model or "").strip():
            return JSONResponse({"error": "model is required for custom OpenAI mode"}, status_code=400)
        ...
        return JSONResponse(
            {"error": f"Generation failed: {type(exc).__name__}: {str(exc)[:200]}"},
            status_code=500,
        )
