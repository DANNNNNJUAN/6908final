...
def _detect_local_ipv4_hosts() -> list[str]:
    hosts: set[str] = set()
    ...
    def _add(host: str):
        ...
        if not value:
            return
        # Ignore IPv6 and obviously invalid values for TrustedHostMiddleware.
        if ":" in value:
            return
        hosts.add(value)
...
def _build_allowed_hosts() -> list[str]:
    ...
    for raw in [*hosts, *_detect_local_ipv4_hosts(), *extra.split(",")]:
