...
async def build_image(
    ...
    *,
    ...
    def has_active_user_llm_cfg(user_llm_cfg: dict | None) -> bool:
        if not isinstance(user_llm_cfg, dict):
            return False
        llm_access_mode = (user_llm_cfg.get("llm_access_mode") or "preset").strip().lower()
        api_key_plain = (user_llm_cfg.get("api_key") or "").strip()
        base_url = (user_llm_cfg.get("base_url") or "").strip()
        ...
        return bool(api_key_plain)

    if (
        user_api_key and user_api_key.strip()
    ) or (
        has_active_user_llm_cfg(current_user_llm_cfg)
    ) or (
        has_active_user_llm_cfg(owner_user_llm_cfg)
    ):
    ...
    elif current_user_id is not None and has_active_user_llm_cfg(current_user_llm_cfg):
        apply_user_llm_cfg(current_user_llm_cfg)
        selected_config_user_id = current_user_id
        usage_source = "current_user_api_key"
    elif has_active_user_llm_cfg(owner_user_llm_cfg):
    ...
    # 检查用户是否走自带模型（BYOK / 自建 OpenAI-compatible）。
    # 注意：API key 现在只从用户级别配置（user_llm_config 表）获取，不再从设备配置读取。
    # 对 custom_openai 来说，只要用户提供了 base_url，就视为自带模型；
    # 本地部署模型通常不需要真实 API key。
    user_provided_api_key = False
    if config:
        # BYOK 有效性判定：
        # - preset：必须有 user_api_key
        # - custom_openai：必须有 llm_base_url（API key 可为空，用于本地模型）
        ...
        if (llm_access_mode == "custom_openai" and has_base) or (
...
def _render_api_key_invalid_image(screen_w: int, screen_h: int) -> Image.Image:
    ...
    message = "API key is invalid or expired. Check device settings."
    try:
...
def _render_quota_exhausted_image(screen_w: int, screen_h: int) -> Image.Image:
    ...
    img = Image.new("1", (screen_w, screen_h), 1)  # 1 = 白色背景
    draw = ImageDraw.Draw(img)
    message = "The owner's free quota is exhausted. Contact the owner."
    try:
        font = load_font("noto_serif_regular", 12)
    except Exception:  # pragma: no cover - 极端环境下回退
        font = None
...
def _render_device_unbound_image(screen_w: int, screen_h: int, pair_code: str) -> Image.Image:
    ...
    draw = ImageDraw.Draw(img)
    title = "Welcome to InkSight"
    pair_line = f"Pair code: {pair_code}" if pair_code else "Generating pair code..."
    hint = "Enter this pair code on the device config page to finish binding."
    try:
