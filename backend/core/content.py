...
_LOCAL_OPENAI_COMPAT_API_KEY = os.getenv("OPENAI_COMPAT_PLACEHOLDER_API_KEY", "local-openai-compat").strip() or "local-openai-compat"
...
def _is_local_openai_compat_base_url(base_url: str | None) -> bool:
    """Best-effort check for local/LAN OpenAI-compatible gateways."""
...
def _get_client(
    ...
    # openai_compat 必须显式提供 base_url，且绝不能回退到平台 provider 的环境变量。
    # 本地部署网关（Ollama / LM Studio / vLLM 等）常常不需要真实 API key，
    # 因此允许空 key，并为 OpenAI SDK 注入一个占位 key。
    if provider == "openai_compat":
        ...
        if not resolved_api_key:
            resolved_api_key = os.getenv("OPENAI_COMPAT_API_KEY", "").strip()
        if not resolved_api_key:
            resolved_api_key = _LOCAL_OPENAI_COMPAT_API_KEY
            if _is_local_openai_compat_base_url(resolved_base_url):
                logger.info(
                    "[LLM] Using placeholder API key for local OpenAI-compatible endpoint %s",
                    resolved_base_url,
                )
            else:
                logger.info(
                    "[LLM] No API key provided for custom OpenAI-compatible endpoint %s; using placeholder key",
                    resolved_base_url,
                )
        return AsyncOpenAI(api_key=resolved_api_key, base_url=resolved_base_url), 120
