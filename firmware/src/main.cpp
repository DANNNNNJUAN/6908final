...
static const int ALERT_PARTIAL_Y0 = (H * 26 / 100);
static const int ALERT_PARTIAL_Y1 = (H * 86 / 100);
...
void loop() {
    ...
    static uint32_t alertFrameChecksum = 0;

    if (focusListening) {
        unsigned long nowMs = millis();
        const unsigned long ALERT_IDLE_INTERVAL_MS = 10000UL;
        const unsigned long ALERT_ANIM_INTERVAL_MS = 5000UL;
        const unsigned long alertIntervalMs = alertVisible ? ALERT_ANIM_INTERVAL_MS : ALERT_IDLE_INTERVAL_MS;
        if (lastAlertPollAt == 0 || nowMs - lastAlertPollAt >= alertIntervalMs) {
            ...
            if (fetchFocusAlertBMP()) {
                uint32_t nextAlertChecksum = computeChecksum(imgBuf, IMG_BUF_LEN);
                if (!alertVisible || nextAlertChecksum != alertFrameChecksum) {
                    if (!alertVisible) {
                        epdDisplayFast(imgBuf);
                    } else {
                        epdPartialDisplay(
                            imgBuf + ALERT_PARTIAL_Y0 * ROW_BYTES,
                            0,
                            ALERT_PARTIAL_Y0,
                            W,
                            ALERT_PARTIAL_Y1
                ...
                alertFrameChecksum = nextAlertChecksum;
                alertVisible = true;
