        ...
        <Card>
          ...
          <CardContent className="space-y-6">
            ...
            <div className="flex gap-2 border-b border-ink/10 pb-2">
              ...
              >
                {tr("使用自定义 / 本地大模型 (BYOK)", "Use Custom / Local LLM (BYOK)")}
              </button>
            </div>
              ...
              <div className="space-y-4">
                <div className="p-3 rounded-sm border border-ink/20 bg-paper-dark">
                  <p className="text-xs text-ink-light">
                    {tr(
                      "💡 在此模式下，设备渲染将不消耗平台免费额度。你可以接第三方 OpenAI-compatible 服务，也可以接本地部署模型（如 Ollama、LM Studio、vLLM）。",
                      "💡 In this mode, device rendering will not consume platform free quota. You can connect either a third-party OpenAI-compatible service or a locally deployed model such as Ollama, LM Studio, or vLLM."
                    )}
                </p>
                </div>
                ...
                <div className="flex gap-2 border-b border-ink/10 pb-2">
                  ...
                  >
                    {tr("自定义 / 本地 OpenAI 兼容", "Custom / Local OpenAI-Compatible")}
                  </button>
                </div>
                  ...
                  <>
                    ...
                    <Field label={tr("Base URL", "Base URL")}>
                      <input
                        ...
                        placeholder={tr("例如：http://127.0.0.1:11434/v1", "e.g. http://127.0.0.1:11434/v1")}
                        className="w-full rounded-sm border border-ink/20 px-3 py-2 text-sm bg-white font-mono"
                      />
                    </Field>
                    <Field label={tr("模型名称", "Model Name")}>
                      <input
                        ...
                        value={llmModel}
                        onChange={(e) => setLlmModel(e.target.value)}
                        placeholder={tr("例如：qwen2.5:7b-instruct 或 deepseek-chat", "e.g. qwen2.5:7b-instruct or deepseek-chat")}
                        className="w-full rounded-sm border border-ink/20 px-3 py-2 text-sm bg-white font-mono"
                      />
                    </Field>
                    <Field label={tr("API Key", "API Key")}>
                      <input
                        ...
                        value={llmApiKey}
                        onChange={(e) => setLlmApiKey(e.target.value)}
                        placeholder={tr("本地模型可留空；远程服务请填写", "Optional for local models; required for hosted services")}
                        className="w-full rounded-sm border border-ink/20 px-3 py-2 text-sm bg-white font-mono"
                        autoComplete="off"
                      />
                    </Field>
                    <p className="text-xs text-ink-light -mt-2">
                      {tr(
                        "如果你的本地 OpenAI-compatible 网关不要求鉴权，这里可以留空。",
                        "If your local OpenAI-compatible gateway does not require authentication, you can leave this blank."
                      )}
                    </p>
