  ...
  return (
    <Card>
      <CardHeader>
        <div className="flex items-center gap-3">
          <CardTitle className="flex items-center gap-2">
            <LayoutGrid size={18} /> {tr("Content Modes", "Content Modes")}
          </CardTitle>
      ...
      <CardContent>
        <ModeGrid
          tr={tr}
          title={tr("Core Modes", "Core Modes")}
          modes={coreModes}
        ...
        <ModeGrid
          tr={tr}
          title={tr("More Modes", "More Modes")}
          modes={extraModes}
          selectedModes={selectedModes}
          onPreview={handleModePreview}
          onApply={handleModeApply}
        ...
        <ModeGrid
          tr={tr}
          title={tr("Custom Modes", "Custom Modes")}
          modes={customModes}
          selectedModes={selectedModes}
          onPreview={handleModePreview}
          onApply={handleModeApply}
          ...
          tailItem={
            <button
              ...
              title={tr("Create custom mode", "Create custom mode")}
            >
              <Plus size={18} className="mb-1" />
              <div className="text-[11px]">{tr("New", "New")}</div>
            </button>
          }
        />
      </CardContent>
  ...
  return (
    <div className="mb-6">
      <div className="flex items-center justify-between gap-2 mb-3 rounded-sm bg-paper-dark border border-ink/10 px-3 py-2">
        ...
        >
          {collapsed ? tr("Expand", "Expand") : tr("Collapse", "Collapse")}
              ...
              <div key={mode} className="rounded-sm border border-ink/10 bg-white overflow-hidden">
                ...
                <div className={`border-t border-ink/10 grid ${onDelete ? "grid-cols-5" : "grid-cols-4"}`}>
                  ...
                  <button
                    onClick={() => onApply(mode)}
                    className="col-span-2 h-9 px-2 text-[11px] sm:text-xs text-ink hover:bg-ink hover:text-white transition-colors flex items-center justify-center gap-1 whitespace-nowrap"
                    title={isSelected ? "Remove from rotation" : "Add to rotation"}
                  >
                    {isSelected ? "-" : "+"}
                  </button>
                  {onDelete ? (
                    <button
                      ...
                      title={tr("Delete mode", "Delete mode")}
                    >
                      <Trash2 size={14} />
                    </button>
                  ) : null}
